...
class MotorGUI(tk.Tk, IGUI):
    def __init__(self, param_client: IParameterClient, status_sub: IStatusSubscriber):
        ...
        self.is_holding = {'motor1': False, 'motor2': False}
        self.hold_step = {'motor1': 0, 'motor2': 0}
        self.current_position = {'motor1': 0.0, 'motor2': 0.0}
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        for idx, motor_name in enumerate(['motor1', 'motor2']):
            ...
            button_plus = ttk.Button(frame, text="+10 deg", style='TButton')
            button_plus.grid(row=1, column=2, pady=5, sticky="ew")
            button_plus.bind('<ButtonPress-1>', lambda event, m=motor_name: self.start_holding(m, 10))
            button_plus.bind('<ButtonRelease-1>', lambda event, m=motor_name: self.stop_holding(m))

            button_minus = ttk.Button(frame, text="-10 deg", style='TButton')
            button_minus.grid(row=1, column=3, pady=5, sticky="ew")
            button_minus.bind('<ButtonPress-1>', lambda event, m=motor_name: self.start_holding(m, -10))
            button_minus.bind('<ButtonRelease-1>', lambda event, m=motor_name: self.stop_holding(m))
    ...
    def start_holding(self, motor, step):
        if not self.reached_status[motor]:
            logging.warning(f"Motor {motor}: Cannot adjust, target not reached yet.")
            self.after(0, lambda: messagebox.showwarning("Warning", f"Motor {motor} is still moving."))
            return
        if self.current_position[motor] <= 5.0 or self.current_position[motor] >= 85.0:
            logging.warning(f"Motor {motor}: Position {self.current_position[motor]} out of safe range (5, 85).")
            self.after(0, lambda: messagebox.showwarning("Warning", f"Motor {motor} at position {self.current_position[motor]} is out of safe range (5, 85)."))
            params = [Parameter(name="rpm", value=ParameterValue(type=ParameterType.PARAMETER_DOUBLE, double_value=0.0))]
            self.send_command(motor, params)
            return
        self.is_holding[motor] = True
        self.hold_step[motor] = step
        rpm = step * 0.1  # Chuyển step (±10 độ) thành RPM (giả định 1 độ/s ≈ 0.1 RPM)
        params = [Parameter(name="rpm", value=ParameterValue(type=ParameterType.PARAMETER_DOUBLE, double_value=rpm))]
        def safe_send():
            success = self.param_client.set_parameters(motor, params)
            if success:
                logging.info(f"Motor {motor}: Sent rpm {rpm} rpm")
            else:
                logging.error(f"Motor {motor}: Failed to send rpm {rpm}")
        self.after(0, safe_send)

    def stop_holding(self, motor):
        self.is_holding[motor] = False
        self.hold_step[motor] = 0
        params = [Parameter(name="rpm", value=ParameterValue(type=ParameterType.PARAMETER_DOUBLE, double_value=0.0))]
        def safe_send():
            success = self.param_client.set_parameters(motor, params)
            if success:
                logging.info(f"Motor {motor}: Stopped continuous movement.")
                self.entries[motor]['targets_str'].set(str(self.current_position[motor]))
            else:
                logging.error(f"Motor {motor}: Failed to stop continuous movement.")
        self.after(0, safe_send)
    ...
    def adjust_target(self, motor, step):
        ...
        new_target = self.current_position[motor] if not current_targets else current_targets[-1]
        new_target += step
        if 0 <= new_target <= 90:
    ...
    def update_status(self, topic, value):
        def safe_update():
            ...
            if param == 'position':
                self.current_position[motor] = value
            if param == 'reached':
