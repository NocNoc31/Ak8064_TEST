...
//           step_(2), target_position_(0.0), reach_count_max_(100),
...
//         // Kiểm tra xem last_data_.position đã được cập nhật
...
//             if (can_->setPositionSpeed(motor_id_, last_data_.position, static_cast<int>(speed_), static_cast<int>(accel_))) {
//                 response << "Stopped at position " << last_data_.position;
//                 RCLCPP_INFO(logger_, "%s", response.str().c_str());
//                 state_publisher_.publishResponse(response.str());
//                 transitionTo(State::MOVING); // Chuyển sang MOVING để xử lý target_positions_
//             } else {
...
//                 if (can_->setPositionSpeed(motor_id_, last_data_.position, static_cast<int>(speed_), static_cast<int>(accel_))) {
//                     response << "Paused at position " << last_data_.position;
//                     RCLCPP_INFO(logger_, "%s", response.str().c_str());
//                     state_publisher_.publishResponse(response.str());
...
//                 float resume_position = (last_command_ == "UP") 
//                     ? last_data_.position + step_ 
//                     : last_data_.position - step_;
//                 if (resume_position > max_angle_) resume_position = max_angle_;
//                 if (resume_position < min_angle_) resume_position = min_angle_;
//                 target_position_ = resume_position;
//                 if (can_->setPositionSpeed(motor_id_, target_position_, static_cast<int>(speed_), static_cast<int>(accel_))) {
//                     response << "Resuming " << last_command_ << " to position " << target_position_;
//                     RCLCPP_INFO(logger_, "%s", response.str().c_str());
//                     state_publisher_.publishResponse(response.str());
//                     transitionTo(State::MOVING);
//                 } else {
//                     response << "Failed to resume " << last_command_ << " to position " << target_position_;
//                     RCLCPP_ERROR(logger_, "%s", response.str().c_str());
//                     state_publisher_.publishResponse(response.str());
//                 }
//             } else {
...
//         // Kiểm tra timeout CMD (1 giây)
//         auto now = node_->now();
//         if (!last_command_.empty() && (now - last_command_time_).seconds() >= 1.0) {
//             RCLCPP_INFO(logger_, "Motor 0x%02X: CMD timeout, clearing last_command_", motor_id_);
//             last_command_ = "";
...
//                     // Gọi setPositionSpeed liên tục với target_position_ trong trạng thái MOVING
//                     can_->setPositionSpeed(motor_id_, target_position_, static_cast<int>(speed_), static_cast<int>(accel_));
//                 } else {
//                     // Xử lý target_positions_ nếu không có lệnh UP/DOWN
...
//                 // Sau khi cập nhật target_positions_, chuyển sang MOVING nếu không có CMD
//                 if (last_command_.empty()) {
//                     transitionTo(State::MOVING);
//                 }
...
public:
    ...
    void control(const MotorData& data) override {
        ...
        auto now = node_->now();
        if (!last_command_.empty() && (now - last_command_time_).seconds() >= 1.0) {
            RCLCPP_INFO(logger_, "Motor 0x%02X: CMD timeout, holding at position %.2f", 
                        motor_id_, last_data_.position);
            target_position_ = last_data_.position;
            can_->setPositionSpeed(motor_id_, target_position_, static_cast<int>(speed_), static_cast<int>(accel_));
            last_command_ = "";
            if (last_command_.empty()) {
    ...
    void setParameters(const std::vector<rcl_interfaces::msg::Parameter>& parameters) override {
        ...
        for (const auto& param : parameters) {
            ...
            if (param.name == "target_positions") {
                ...
                RCLCPP_WARN(logger_, "Motor 0x%02X: Unknown command: %s", motor_id_, param.name.c_str());
            }
        }
    }
